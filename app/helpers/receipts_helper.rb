module ReceiptsHelper
  ...
  def self.generate_receipt(receipt, order)
  ...
  def self.order_line_items(order)
    line_items = [
      ["<b>Item</b>", "<b>QTY</b>", "<b>Rate/Unit</b>", "<b>Amount</b>"],
      ["<b>Rubber Stamp</b>", nil, nil, nil, nil]
    ]
    order.order_items.each do |item|
      line_items << [item.name, item.qty, item.amount / item.qty, item.amount]
    end
    line_items << [nil, nil,  "<b>Total</b>", order.total_item_amount]
    line_items
  end
